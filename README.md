...
The app listens on `0.0.0.0` (not only localhost) and uses `PORT` (default `3000`). The first registered account is a normal `MEMBER`; promote trusted operators directly in the database or through an administrative provisioning process before exposing management endpoints.
